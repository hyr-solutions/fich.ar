<script lang="ts">
	...
	import { formatUrlInput, pb, sizeToInputContent } from '$lib'
...
<section class="mx-auto flex w-full max-w-5xl flex-col text-white">
	...
	<form
		oninput={async (e) => {
			...
			} else sizeToInputContent(schemaCheckSiteInputElement)
		}}
		onsubmit={async (e) => {
			e.preventDefault()
			const t = e.currentTarget
			...
			if (isProd && siteUrl) {
				...
				if (res?.ok) {
					...
					const newForm = await pb
						.collection('forms')
						.update<FormsResponse<{ schema: SchemasResponse }>>(
							form.id,
							{
								is_dev,
								schema_check_site
					...
					if (form.expand?.schema && newForm.expand?.schema)
						form.expand.schema = newForm.expand.schema
		...
		{#if form.schema}
			<label
				...
				<button
					...
					<span class="icon-[lucide--loader-circle] hidden animate-spin text-2xl group-wait:block"
					></span>
				</button>
				<span
					class="hidden items-center gap-2 whitespace-nowrap group-sent:group-has-[:checked]/prod:flex">
		...
		<label
			...
			<span class="block whitespace-nowrap group-has-[:checked]/dev:hidden">
				Switch to Development
			</span>
			<span class="hidden whitespace-nowrap group-has-[:checked]/dev:block"> In Development </span>
			<span
				class="icon-[lucide--flask-conical] rotate-12 text-2xl group-has-[:checked]/dev:rotate-0"
			></span>
		</label>
	</form>
</section>
<section class="flex justify-center">
	<section class="mx-auto flex min-w-[72rem] flex-col text-white">
		...
		{#if form.schema}
			...
			<div class="mt-16 flex h-full gap-8">
				<div class="flex min-h-full flex-col gap-2 font-medium">
					{#each [['Submissions', 'icon-[lucide--list-start]'], ['Forwarding', 'icon-[lucide--arrow-big-right-dash]'], ['Checks', 'icon-round-[lucide--app-window-mac]'], ['Utilities', 'icon-[lucide--drill]']] as [href, icon]}
						<a
							data-cursor-hover
							...
							{$page.url.pathname.includes(href.toLowerCase())
		...
		{:else}
			<div class="mt-24">
				<p class="mb-4">
					Insert the following <code class="rounded bg-slate-600 p-1">iframe</code> snippet wherever
					you'd like your form to appear on your website. Once we receive your first submission, additional
