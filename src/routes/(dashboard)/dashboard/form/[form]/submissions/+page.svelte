<script lang="ts">
	import { containsSubmissionData, type ParsedField } from '$lib/form'
	...
	let schemaData = $derived(
		(form.expand?.schema.json as ParsedField[]).filter(containsSubmissionData) as ParsedField[]
	)
</script>

{#if submissions.items.length}
	<Cursor attributeName="data-cursor-table-rows" padding={2} />
{/if}
<table class="w-full table-auto text-start">
	<thead class="font-semibold text-white">
		<tr class="">
			{#each schemaData as field}
				<th class="whitespace-nowrap p-4 pt-3 text-start">
					{field.name}
				</th>
			{/each}
			<th class="p-4 pt-3 text-start"> Submitted </th>
			<th class="flex justify-end gap-2 p-4 pt-3 text-slate-600">
				<a
	...
	<tbody class="group font-['Space_Mono'] text-slate-400">
		{#each submissions.items as submission}
			...
			{@const firstDate =
				submissions.items.find((i) => formatRelativeTime(i.created) === submittedDate)?.id ===
				submission.id}
			<tr
				...
				<td
					class="border-b-0 border-transparent px-4 group-hover/row:border-white group-hover/row:font-bold">
